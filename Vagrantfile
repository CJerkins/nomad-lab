...
Vagrant.configure(2) do |config|
    config.vm.box = "generic/ubuntu1804" # 16.04 LTS
    ...
    # Servers
    1.upto(1) do |i|
        config.vm.define "nomad-a-#{i}s" do |n|
    ...
    # Clients
    1.upto(1) do |i|
        config.vm.define "nomad-a-#{i}c" do |n|
        n.vm.provision "shell", inline: $script
        ...
        if i == 1
          ...
          n.vm.hostname = "nomad-a-#{i}c"
          n.vm.network "private_network", ip: "172.16.2.#{i+100}"
          n.vm.network "forwarded_port", guest: 4646, host: 4646, auto_correct: true
        end  
    end
end
